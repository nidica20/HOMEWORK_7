		...
		if (!the_queue.empty()) {
			...
			if (plane->ready_takeoff_time == -1) { // new plane has arrived at the front
				...
				if (clock - plane->ready_takeoff_time > departure_time) {
					...
					int departing_wait_time = 0; //initialize the variable
					departing_wait_time = clock - plane->enter_departure_time; //setting value based on wait time calculation

					// FIXME: update total_wait and num_served
					total_wait += departing_wait_time; //adds up all wait times in the departure queue
					num_served++; //increments every time an airplane is served
