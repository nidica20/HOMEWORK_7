...
public:
	...
	void update(int clock) 
	{
		...
		*/
		...
		if (my_random.next_double() < arrival_rate) //condition compares psuedoraondom number to arrival_rate
		{
			the_queue.push(new Plane(clock));
			//since values are being compared, the probability of condition being true is proportional to arrival_rate value
			// If arrival_rate is 0.4, then condition is true 40% of time (as described on page 393).
		}
